...
\begin{lstlisting}[language=SQL, caption={Implementacion de entidades de usuario en la base de datos}]
-- Estatus (catalogo compartido, debe crearse primero)
...
CREATE TABLE Tutor (
    ...
    -- id_tutor hereda el id_usuario de la tabla Usuarios
...
\begin{lstlisting}[language=SQL, caption={Implementación de entidades del proceso de evaluación}]
...
\begin{lstlisting}[language=SQL, caption={Restricciones de unicidad en campos críticos}]
-- Unicidad de nombre de usuario en Usuarios
CONSTRAINT UQ_Usuarios_usuario 
    UNIQUE (usuario),

-- Unicidad de correo electrónico en Tutor
CONSTRAINT UQ_Tutor_correo 
    UNIQUE (correo),

-- Un alumno solo puede tener un intento por ejercicio asignado
CREATE UNIQUE INDEX UQ_Intentos_original
ON Intentos (id_alumno, id_ejercicio_tutor)
WHERE id_recomendacion IS NULL;

-- Un tutor no puede asignar el mismo ejercicio si ya tiene una asignacion activa. 
CREATE UNIQUE INDEX UQ_ET_activo
    ON Ejercicio_Tutor (id_tutor, id_ejercicio)
    WHERE id_estatus = 1;

-- Relación 1:1 entre Intento y Análisis ortográfico
CONSTRAINT UQ_Analisis_Ort_intento 
    UNIQUE (id_intento),

-- Relación 1:1 entre Intento y Análisis caligráfico
...
BEGIN
    ...
    -- Obtener el alumno del intento recién analizado
    SELECT @id_alumno = i.id_alumno
    FROM inserted ins
    INNER JOIN Intentos i ON i.id_intento = ins.id_intento;

    -- Calcular el nuevo promedio ortográfico del alumno
    ...
    -- Insertar o actualizar según si ya existe el registro
    IF EXISTS (
...
BEGIN
    ...
    -- Calcular los nuevos promedios caligráficos del alumno
    SELECT
